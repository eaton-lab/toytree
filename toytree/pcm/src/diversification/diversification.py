...
def get_tip_level_diversification(
    ...
    **kwargs: dict
    ...
    return calculate_posterior(_get_tip_level_diversification, trees, njobs, **kwargs)
...
def get_prob_dist_of_ntips(
    ...
    def alpha(time):
        return (rel_ext * ((np.exp(net_div) * time) - 1)) / (
            np.exp(net_div) * time - rel_ext
        )

    # ...
    def beta(time):
        return (np.exp(net_div) * time - 1) / (np.exp(net_div) * time - rel_ext)
