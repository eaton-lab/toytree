...
class SubstitutionModel(ABC):
    ...
    def __init__(self):
        ...
        self._p_matrices = []
        """: probability matrices from symbolic to calculation-friendly forms."""
...
class JC69(SubstitutionModel):
    ...
    def _update_p_matrix(self):
        ...
        self._p_matrices.append(
            self._p_matrices[0].subs(self._mu * self._t, self._d / 3)
        )

    def _update_pairwise_likelihood_formula(self):
        """Set the pairwise likelihood and log-likelihood formulas.

        Considering multinomial distribution for all 16 site patterns,
        the probability is ``p_0 / 4`` for any constant site pattern
        and ``p_1 / 4`` for any different site pattern, because these
        site-pattern classes each apply to all four bases.
        """
        p_matrix = self.p_matrix()
        self._pairwise_likelihood_formula = (
            p_matrix[0, 1] * self._PI[1]
        ) ** self._x * (p_matrix[0, 0] * self._PI[0]) ** (self._n - self._x)
        self._pairwise_loglikelihood_formula = self._x * sympy.log(
            p_matrix[0, 1] * self._PI[1]
    ...
    def get_pairwise_loglike_function(
        ...
        if (x_obs and n_obs) or change_matrix is not None:
            ...
            return sympy.lambdify(
                [self._d], loglike_formula.evalf(subs={self._n: n_obs, self._x: x_obs})
            )
        raise TypeError("Please input change_matrix or x_obs and n_obs!")

    def get_p_matrix_function(self) -> Callable:
        """Return a fast transition-probability function.
        ...
        -------
        sympy.core.function
            Function that takes phylogenetic distance ``d`` and returns
        ...
        def _p_matrix_quick_func(_d, _kappa):
            (
                p0,
                p1,
            ) = p0_f(_d, _kappa), p1_f(_d, _kappa)
            return np.array(
                [[p0, p1, p1, p1], [p1, p0, p1, p1], [p1, p1, p0, p1], [p1, p1, p1, p0]]
            )
...
class K80(SubstitutionModel):
    ...
    def __init__(self):
        ...
        """: symbolic transition-rate variable, e.g. A <-> G and C <-> T"""
        self._beta = beta = sympy.Symbol("beta")
        """: symbolic transversion-rate variable, e.g. A/G <-> C/T"""
        self._kappa = sympy.Symbol("kappa_ls")
        """: symbolic transition/transversion ratio, i.e. alpha / beta"""
        self._d = sympy.Symbol("d")
        """: symbolic phylogenetic distance, where d=(alpha+2*beta)*t"""
        self._n = sympy.Symbol("n")
    ...
    def _update_p_matrix(self):
        ...
        # 1. Express alpha and beta in terms of phylogenetic distance,
        # timespan, and kappa.
    ...
    def _update_pairwise_likelihood_formula(self):
        ...
        Under the multinomial model for the 16 site patterns, the
        probabilities are ``p_0 / 4`` for constant patterns,
        ``p_1 / 4`` for transition patterns, and ``p_2 / 4`` for
        transversion patterns.
        """
        p_matrix = self.p_matrix()
        self._pairwise_likelihood_formula = (
    ...
    def get_pairwise_loglike_function(
        ...
        """Return a callable log-likelihood function for K80 data.

        If ``s_obs`` / ``v_obs`` / ``n_obs`` are not provided, they are
        derived from ``change_matrix``.

        Parameters
        ----------
        change_matrix: np.array
            Observed matrix of pairwise base changes. For example,
            Yang (2014) uses::
        ...
        s_obs : int
            Observed transition sites.
        v_obs : int
            Observed transversion sites.
        n_obs : int
            Observed total number of sites.
        ...
        -------
        sympy.core.function
            Function that takes phylogenetic distance ``d`` and K80
            ``kappa`` and returns a log-likelihood value.
        """
        if (s_obs and v_obs and n_obs) or change_matrix is not None:
            ...
            return sympy.lambdify(
                [self._d, self._kappa],
                loglike_formula.evalf(
                    subs={self._n: n_obs, self._s: s_obs, self._v: v_obs}
                ),
            )
        else:
            raise TypeError("Please input change_matrix, or s_obs, v_obs and n_obs!")

    def get_p_matrix_function(self, kappa: float = 2.0):
        """Return a fast K80 transition-probability function.

        Parameters
        ----------
        kappa: float
            Transition / transversion ratio. The resulting function is
            about 15% faster when ``kappa`` is fixed.

        Returns
        -------
        sympy.core.function
            Function that takes phylogenetic distance ``d`` and
            returns a transition-probability matrix.
        """
        # This is about 3x faster than
        # `sympy.lambdify([self._d, self._kappa], self.p_matrix())`
        # and still modestly faster than a direct formula implementation.
        p_matrix = self.p_matrix()
        p0_f = sympy.lambdify(
            [self._d], p_matrix[0, 0].subs(self._kappa, kappa), modules="numpy"
        )
        p1_f = sympy.lambdify(
            [self._d], p_matrix[0, 1].subs(self._kappa, kappa), modules="numpy"
        )
        p2_f = sympy.lambdify(
            [self._d], p_matrix[0, 2].subs(self._kappa, kappa), modules="numpy"
        )
...
class TN93(SubstitutionModel):
    ...
    def __init__(self):
        super().__init__()
        self._PI = pi_1, pi_2, pi_3, pi_4 = sympy.symbols(
            ["pi_" + base_type for base_type in BASE_ORDER]
        )
        """: symbolic equilibrium-frequency variables for the nucleotides"""
        ...
        self._d = sympy.Symbol("d")
        """: symbolic phylogenetic distance, where d=(alpha+2*beta)*t"""
        self._change_mtx = sympy.Matrix(
            [["O_" + from_b + to_b for from_b in BASE_ORDER] for to_b in BASE_ORDER]
        )
        """: the symbolic matrix to store observed change of matrix"""
        self._q_matrix = sympy.Matrix(
            [
                [
                    -(pi_2 * alpha_1 + pi_3 * beta + pi_4 * beta),
                    pi_1 * alpha_1,
                    pi_1 * beta,
                    pi_1 * beta,
                ],
                [
                    pi_2 * alpha_1,
                    -(pi_1 * alpha_1 + pi_3 * beta + pi_4 * beta),
                    pi_2 * beta,
                    pi_2 * beta,
                ],
                [
                    pi_3 * beta,
                    pi_3 * beta,
                    -(pi_1 * beta + pi_2 * beta + pi_4 * alpha_2),
                    pi_3 * alpha_2,
                ],
                [
                    pi_4 * beta,
                    pi_4 * beta,
                    pi_4 * alpha_2,
                    -(pi_1 * beta + pi_2 * beta + pi_3 * alpha_2),
                ],
            ]
        )
        """: the rate matrix"""
    ...
    def _update_express(self):
        self._express = sympy.solve(
            ...
            [self._alpha_1, self._alpha_2, self._beta],
        )
    ...
    #     # --- use d and kappa_ls to replace alpha and beta ---
    #     # 1. Express alpha_1, alpha_2, and beta using phylogenetic
    #     # distance, timespan, kappa_1, and kappa_2.
    ...
    def _update_p_matrix(self):
        """Append cached TN93 transition matrices.

        These closed-form matrices are stored explicitly instead of
        evaluating ``(self._q_matrix * self._t).exp()`` each time.
        """
        ...
        self._p_matrices.append(
            sympy.Matrix(
                [
                    [
                        (pi3 * pi1 + pi4 * pi1) * exp(-b * t) / pi12
                        + pi2 * exp(-a1 * pi12 * t - b * pi34 * t) / pi12
                        + pi1,
                        (pi3 * pi1 + pi4 * pi1) * exp(-b * t) / pi12
                        - pi1 * exp(-a1 * pi12 * t - b * pi34 * t) / pi12
                        + pi1,
                        pi1 - pi1 * exp(-b * t),
                        pi1 - pi1 * exp(-b * t),
                    ],
                    ...
                    [
                        ...
                        -pi3 * exp(-b * t) + pi3,
                        pi3
                        + pi3 * pi12 * exp(-b * t) / pi34
                        - exp(-a2 * pi34 * t - b * pi12 * t) / (-pi3 / pi4 - 1),
                        pi3
                        - pi3 * exp(-a2 * pi34 * t - b * pi12 * t) / pi34
                        + pi3 * pi12 * exp(-b * t) / pi34,
                    ],
                    [
                        -pi4 * exp(-b * t) + pi4,
        ...
        self._p_matrices.append(
            sympy.Matrix(
                [
                    [
                        pi1
                        + (pi2 * exp(d * (-k1 * pi12 - pi34 + 1) / c1) + pi1 * pi34)
                        * exp(-d / c1)
                        / pi12,
                        pi1
                        + pi1
                        * (pi34 - exp(d * (-k1 * pi12 - pi34 + 1) / c1))
                        * exp(-d / c1)
                        / pi12,
                        pi1 - pi1 * exp(-d / c1),
                        pi1 - pi1 * exp(-d / c1),
                    ...
                    [
                        ...
                        * exp(-d / c1)
                        / pi12,
                        pi2
                        + (pi2 * pi34 + pi1 * exp(d * (-k1 * pi12 - pi34 + 1) / c1))
                        * exp(-d / c1)
                        / pi12,
                        pi2 - pi2 * exp(-d / c1),
                        pi2 - pi2 * exp(-d / c1),
                    ...
                    [
                        ...
                        pi3 - pi3 * exp(-d / c1),
                        pi3
                        + (pi3 * pi12 + pi4 * exp(d * (-k2 * pi34 - pi12 + 1) / c1))
                        * exp(-d / c1)
                        / pi34,
                        pi3
                        + pi3
                        * (pi12 - exp(d * (-k2 * pi34 - pi12 + 1) / c1))
                        * exp(-d / c1)
                        / pi34,
                    ],
                    [
                        ...
                        * exp(-d / c1)
                        / pi34,
                        pi4
                        + (pi3 * exp(d * (-k2 * pi34 - pi12 + 1) / c1) + pi4 * pi12)
                        * exp(-d / c1)
                        / pi34,
                    ],
                ]
    ...
    def _update_pairwise_likelihood_formula(self):
        """Set the pairwise likelihood formulas for TN93."""
        # np.array and sympy.Matrix has different multiplication scheme
        self._p_matrix_w = np.array(self.p_matrix()) * np.array(self._PI)
        self._log_p_matrix_w = np.array(
            sympy.simplify(sympy.Matrix(self._p_matrix_w).applyfunc(sympy.log))
        )
        self._pairwise_likelihood_formula = self._p_matrix_w ** np.array(
            self._change_mtx
        )
        self._pairwise_loglikelihood_formula = (
            np.array(self._change_mtx) * self._log_p_matrix_w
        )

    def get_pairwise_loglike_function(
        self, change_matrix: ArrayLike
    ) -> sympy.core.function:
        """Return a callable log-likelihood function for TN93 data.
        ...
        -------
        sympy.core.function
            Function that takes phylogenetic distance ``d``,
            ``kappa_1``, ``kappa_2``, and equilibrium frequencies and
            returns a log-likelihood value.
        """
        if self._p_matrix_w is None:
            self._update_pairwise_likelihood_formula()
        # A direct symbolic likelihood formula is slow when too many
        # parameters remain free. Use the logged weighted matrix form
        # instead and contract it with the observed change counts.
        like_formula = (self._log_p_matrix_w * change_matrix).sum()
        return sympy.lambdify(
            [self._d, self._kappa_1, self._kappa_2] + self._PI, like_formula
        )

    # TODO using Horner's rule may help speed up, i.e. combining terms in common
    def get_p_matrix_function(
        self,
        kappa_ls: tuple[float, float] = None,
        pi_ls: tuple[float, float, float, float] = None,
    ):
        """Return a fast TN93 transition-probability function.

        The resulting function is faster when more parameters are fixed.

        Parameters
        ----------
        kappa_ls: tuple[float, float]
            Values to which ``kappa_1`` and ``kappa_2`` are fixed.
        pi_ls: tuple[float, float, float, float]
            Values to which nucleotide equilibrium frequencies are fixed.

        Returns
        -------
        sympy.core.function
            Function that takes the remaining free parameters and
            returns a transition-probability matrix.
        """
        # This is much faster than lambdifying the full symbolic matrix
        # every time and then substituting fixed parameters afterward.
        p_matrix = self.p_matrix()
        if kappa_ls is not None and pi_ls is not None:
            ...
            p_matrix = p_matrix.evalf(
                subs={self._PI[go_b]: pi_val for go_b, pi_val in enumerate(pi_ls)}
            )
            return sympy.lambdify([self._d, self._kappa_1, self._kappa_2], p_matrix)
        else:
            return sympy.lambdify(
                [self._d, self._kappa_1, self._kappa_2] + self._PI, p_matrix
            )
...
def combine_descendent_conditional_probability(
    ...
    child_conditional_prob_gen : Iterator[ArrayLike in the shape of (4,)]
        Iterator that yields one child's conditional probability at a
        time. Each array should have ``shape=(4,)``, for example
        ``np.array([0.05, 0.001, 0.04, 0.002])``.
    child_prob_matrix_gen: Iterator[ArrayLike in the shape of (4,4)]
        Iterator that yields one child's transition matrix at a time.
        Each matrix should have ``shape=(4, 4)``, for example::

            np.array(
                [[0.8644351, 0.06591888, 0.03482301, 0.03482301],
        ...
        This example is a K80 matrix with ``d=0.15`` and ``k=2``.
        Such matrices can be generated by ``get_p_matrix_function()``
        on the corresponding substitution-model object.

    Returns
    -------
    ArrayLike[float]
        Conditional probability of the combined descendants as a
        NumPy array with ``shape=(4,)``.
    """
    accumulated_prob = (
        next(child_conditional_prob_gen) * next(child_prob_matrix_gen)
    ).sum(axis=1)
...
def get_tree_likelihood(
    ...
    for nidx in range(tree.ntips):
        ...
        assert (
            node.name in data
        ), f"data must include all tips in the tree. Missing={node.name}"
...
def combine_descendent_conditional_probability_with_plot(
    ...
    for go_base in range(4):
        # conditional probs
        x_coords_c = [
            40 - (1.5 - go_base) * AX1_x_width for foo in child_conditional_prob_list
        ]
        y_coords_c = [
            20 - (0.5 - go_c) * AX1_y_width
        ...
        ax1.scatterplot(
            ...
            marker=[
                toyplot.marker.create(
                    ...
                    size=11,
                    mstyle={
                        "fill": BASE_COLOR_SCHEME[go_base],
                        "stroke": "lightgrey",
                        "fill-opacity": 0.2,
                    },
                )
        ...
        # p matrix
        x_coords_m = [
            40 - (1.5 - _to_base) * AX1_x_width
            for foo in child_conditional_prob_list
            for _to_base in range(4)
        ]
        y_coords_m = [
            20 - (0.5 - go_c) * AX1_y_width - (3 + go_base) * AX1_y_width_m
            for go_c, foo in enumerate(child_conditional_prob_list)
            for _to_base in range(4)
        ]
        ax1.scatterplot(
            ...
            marker=[
                toyplot.marker.create(
                    ...
                    size=11,
                    mstyle={
                        "fill": BASE_COLOR_SCHEME[_to_base],
                        "stroke": "lightgrey",
                        "fill-opacity": 0.2,
                    },
                )
        ...
        # heads of p matrix
        labels = [
            [from_to + _base for _base in BASE_ORDER] for from_to in ("to ", "from ")
        ]
        ax1.scatterplot(
            [
                40
                + from_to * AX1_x_width * 2.5
                - (1.5 - go_base) * AX1_x_width * (1 - from_to)
                for foo in child_conditional_prob_list
                for from_to in range(2)
            ],
            [
                20
                - (0.5 - go_c) * AX1_y_width
                - (3 + go_base) * AX1_y_width_m * from_to
                - 2 * (1 - from_to) * AX1_y_width_m
                for go_c, foo in enumerate(child_conditional_prob_list)
                for from_to in range(2)
            ],
            size=15,
    ...
    for go_child, child_conditional_probability in enumerate(
        child_conditional_prob_list
    ):
        intermediate_matrices.append(
            child_conditional_probability * child_prob_matrix_list[go_child]
        )
    # plot the intermediate matrices
    for go_base in range(4):
        x_coords_m = [
            23.5 - (1.5 - _to_base) * AX1_x_width
            for foo in child_conditional_prob_list
            for _to_base in range(4)
        ]
        y_coords_m = [
            20 - (0.4 - go_c) * AX1_y_width - (3 + go_base) * AX1_y_width_m
            for go_c, foo in enumerate(child_conditional_prob_list)
            for _to_base in range(4)
        ]
        ax1.scatterplot(
...
def node_conditional_probability_with_plot(
    ...
    if node.is_leaf():
        ...
        for child_node in node.children:
            child_generator = node_conditional_probability_with_plot(
                child_node, p_matrix_func, tree
            )
        ...
        ax1.show = False
        # plot tree
        edge_colors = ["black"] * tree.nnodes
        for child_node in node.children:
            edge_colors[child_node.idx] = "red"
        tree.draw(
            node_labels=True,
            node_sizes=12,
        ...
        for go_base in range(4):
            ...
            accessible_probs = [
                j for go_p, j in enumerate(candidate_probs) if accessible_points[go_p]
            ]
...
def get_tree_likelihood_plot_gen(
    ...
    for sp_name, sp_state in tip_states.items():
        sp_node = tree.get_nodes(sp_name)[
            0
        ]  # do we have better function to do this in toytree?
        sp_node.conditional_prob = (np.array(BASE_ORDER) == sp_state).astype(float)
    node_cp_plot_gen = node_conditional_probability_with_plot(
        tree.treenode, p_matrix_func=p_matrix_func, tree=tree
    )
...
if __name__ == "__main__":
    TEST_ARRAY = np.array(
        [[179, 23, 1, 0], [30, 219, 2, 0], [2, 1, 291, 10], [0, 0, 21, 169]]
    )
    OBSERVED_DATA = {"sp-A": "T", "sp-B": "C", "sp-C": "A", "sp-D": "C", "sp-E": "C"}
    TREE = toytree.tree(
        "(((sp-A:0.2,sp-B:0.2):0.1,sp-C:0.2):0.1,(sp-D:0.2,sp-E:0.2):0.1);"
    )
    # TEST_MODEL = K80()
    TEST_MODEL = JC69()
    TEST_MODEL.p_matrix()
    ...
    # LIK = get_tree_likelihood(
    #     tree=TREE,
    #     data=OBSERVED_DATA,
    #     p_matrix_func=TEST_MODEL.get_p_matrix_function(kappa=2.0),
