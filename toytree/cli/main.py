...
def setup_parsers() -> argparse.ArgumentParser:
    ...
    parser_draw = subparsers.add_parser("draw", help="create tree drawing")
    parser_draw.add_argument("-ts", type=str, default="n", 
        metavar="treestyle", help="tree style")
    parser_draw.add_argument("-d", type=int, metavar="dim", nargs=2, 
        default=(None, None), help="width height (px)")    
    parser_draw.add_argument("-o", type=Path, metavar="basename", 
        default=TMPFILE, help="output basename[.format suffix]")
    parser_draw.add_argument("-v", type=str, metavar="app", nargs="?", 
        const="auto", help="open file with default browser or app.")
    parser_draw.add_argument("-f", type=str, help="output file format", 
        choices=["html", "svg", "pdf"], default="html"
    )
    parser_draw.add_argument("TREE", type=string_or_stdin_parse, help="tree newick file or string",
    )
    ...
    parser_distance.add_argument("-m", type=str, 
        choices=['rf', 'rfi', 'rfj', 'qrt'],
        default='rf',
        help="distance metric method")
    parser_distance.add_argument("-n", "--normalize", action="store_true",
