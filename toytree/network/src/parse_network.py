...
class NetworkToMajorTree:
    ...
    ----------
    disconnect: bool
        If False then destination nodes are left in the parsed tree
        as mono nodes.
    """
    def __init__(self, net: Union[str, Path]): #, disconnect=True
        self.net: str = None #self._get_network_as_newick_str(net)
        ...
        self._set_net_as_newick_str(net)
        # self.disconnect: bool = disconnect

    def _set_net_as_newick_str(self, net: Union[str, Path]) -> str:
...
def test3B():
    ...
    parse = NetworkToMajorTree(net)#, disconnect=False)
    tree, admix = parse.get_major_tree_and_admix_edges()
    return tree, admix
...
def test_interactive():
    ...
    tree, admix = parse_network_to_tree_and_admix(hnewick)
    return tree, admix
