...
class TestRootByMinimalAncestorDeviation(PytestCompat):
    def setUp(self):
        ...
        # baltree now requires an even number of tips.
        self.btree = toytree.rtree.baltree(26)
    ...
    def test_dev0_trees(self):
        for tree in [self.utree, self.itree, self.btree]:
            rtree, stats = tree.mod.root_on_minimal_ancestor_deviation(
                return_stats=True
            )
            mad_results = {
                "minimal_ancestor_deviation": 0.0,
                "root_ambiguity_index": 0.0,
                "root_clock_coefficient_of_variation": 0.0,
            }
            with self.subTest(tree=tree.ntips):
                for key, stat in stats.items():
                    self.assertAlmostEqual(stats[key], mad_results[key])

    def test_tree1_unrooted(self):
        rtree, stats = self.tree1.mod.root_on_minimal_ancestor_deviation(
            return_stats=True
        )
        mad_results = {
    ...
    def test_tree1_rooted_terminal(self):
        rtree, stats = self.tree1.root("a").mod.root_on_minimal_ancestor_deviation(
            return_stats=True
        )
        mad_results = {
            "minimal_ancestor_deviation": 0.3281920104298636,
    ...
    def test_tree1_rooted_internal(self):
        rtree, stats = self.tree1.root("X").mod.root_on_minimal_ancestor_deviation(
            return_stats=True
        )
        mad_results = {
            "minimal_ancestor_deviation": 0.3281920104298636,
    ...
    def test_tree1_rooted_on_non_optimal(self):
        ...
        for key, stat in stats.items():
            self.assertAlmostEqual(stats[key], mad_results[key])
        self.assertEqual(
            rtree.get_topology_id(), self.tree1.root("a").get_topology_id()
        )

    def test_tree2_unrooted(self):
        rtree, stats = self.tree2.mod.root_on_minimal_ancestor_deviation(
            return_stats=True
        )
        mad_results = {
    ...
    def test_tree2_rooted_terminal(self):
        rtree, stats = self.tree2.root("a").mod.root_on_minimal_ancestor_deviation(
            return_stats=True
        )
        mad_results = {
            "minimal_ancestor_deviation": 0.4011968627201758,
    ...
    def test_tree2_rooted_internal(self):
        rtree, stats = self.tree2.root("X").mod.root_on_minimal_ancestor_deviation(
            return_stats=True
        )
        mad_results = {
            "minimal_ancestor_deviation": 0.4011968627201758,
